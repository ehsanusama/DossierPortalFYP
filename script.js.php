<script>
	...
	$(function () {
		var response_module = ["login", "forgot_password_module", "register_module", "", "user_business_shift",
			"assign_user_roles", 'register_staff_module', 'send_billing_notification', 'add_time_off',
			'cancel_billing', 'renew_subscription', 'add_subscription', 'leave_management', 'business_module','task_module'
		];
				...
				success: function (response, text) {
					...
					if (response_module.includes(json.action)) {
						...
						if ((json.action == "register_staff_module" || json.action ==
								...
								'renew_subscription' || json.action == 'add_subscription' ||
								json.action == "leave_management" || json.action ==
								"business_module"  || json.action ==
								"task_module") && json.sts == "success") {
							setTimeout(function () {
								$(".modal").modal('hide');
	...
	$(document).on("click", ".attendance_delete_btn", function () {
		...
		if(confirm("Do you want to delete attendance record ?")){
			...
			type: "post",
			dataType: 'text',
			data: {action:"delete_attendance",att_id:att_id},
			beforeSend: function () {
				$(".sticky-response").html('<div class="alert alert-info">Please Wait</div>');
			},
			success: function (response, text) {
...
var dateTime = date+'_'+time;
      var opt = {
        ...
        image:        { type: 'jpeg', quality: 0.98 },
        html2canvas:  { scale: 2 },
        jsPDF:      { orientation: 'l', unit: 'pc', format: 'a4', putOnlyUsedFonts:true, floatPrecision: 16} 
      };
      html2pdf().set(opt).from(element).save();
